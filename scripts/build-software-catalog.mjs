...
for(const item of items){
  ...
  for(const download of item.downloads){
    ...
    if(download.url){
      ...
      if(!/^[a-f0-9]{64}$/i.test(download.sha256||''))throw new Error(`${item.slug}: direct download requires SHA-256`);
    }else{
...
const sourceBadge=item=>item.downloads.some(download=>download.url)?'✓ SHA-256 из Microsoft WinGet':'✓ Официальный GitHub Release';
